...
MeiGen offers three video models: **Seedance 2.0** (per-second pricing, 4–15 seconds), **Happyhorse 1.0** (per-second, 3–15 seconds), and **Veo 3.1** (per-generation, 4 / 6 / 8 seconds, two tiers, audio always on). Open the video sidebar from the **Generate Video** button in the floating dock.
...
| **Resolution** | 480p, 720p, 1080p (default 480p) | 480p saves credits |
...
Extension uses a separate With-reference-video rate table (Fast 480p 8 credits/sec, 720p 14 credits/sec; Pro 720p 16 credits/sec, 1080p 28 credits/sec). **Billable seconds = reference video duration + your selected duration** (not less than the minimum-billable floor). The full formula, lookup table, and examples are in [Models → Seedance 2.0](/en/features/models).

Typical costs (examples):
- Upload a 3-second reference + generate 5 seconds at 480p: **72 credits** (billable = max(3+5, 9) = 9 sec × 8)
- Upload a 5-second reference + generate 10 seconds at 720p: **238 credits** (billable = max(5+10, 17) = 17 sec × 14)
...
| Model | Typical Time |
| --- | --- |
| Z Image Turbo | ~12 seconds |
| Seedream 4.5 | ~17 seconds |
| Seedream 5.0 Lite, Nanobanana 2 | ~35–38 seconds |
| GPT Image 2.0, Nanobanana Pro | ~45 seconds |
| Midjourney V8.1 | ~45 seconds |
| Happyhorse 1.0 (video) | ~1–2 minutes |
| Seedance 2.0 (video) | ~1–2 min (Mini, default) / ~1–3 min (Fast) / ~2–5 min (Pro, longer at 4K) |
...
- **Daily credits**: refresh every 24 hours, used for **basic models** (currently only Z Image Turbo).
- **Purchased credits**: obtained by topping up, never expire, usable on all models.

Generating with premium models (GPT Image 2.0, Nanobanana, Seedream, Midjourney, Seedance, Veo, etc.) requires purchased credits.
