...
description: "Connect to the hosted MCP server over HTTP without installing a local package."
---
...
The example costs 6 credits at 2K Standard. Add public reference-image URLs with `referenceImages` for editing or visual guidance. Use a fresh `attemptId` (8–128 characters) for a new generation; reuse it only when retrying the exact same request after a failure or timeout. After receiving a generation ID, use `check_generation` to continue tracking it. See [model pricing](/en/features/models) for the full matrix.
