---
title: "Setup"
description: "Install and configure the MeiGen MCP plugin for your editor and provider"
---
...
<Steps>
  ...
  <Step title="Add configuration">
    Add the following to your MCP configuration. Include the environment variables for your chosen provider(s):
    ...
    Only include the env vars for the providers you want to use. You don't need all three.
  </Step>
...
<Steps>
  ...
  <Step title="Add configuration">
    Use the same JSON format as Cursor above.
  </Step>
...
Configure one or more providers. When multiple are available, the plugin selects in order: MeiGen → ComfyUI → OpenAI. You can override this per-request.
...
Use OpenAI's image models or any OpenAI-compatible API service.

| Variable | Value |
| --- | --- |
| `OPENAI_API_KEY` | Your API key starting with `sk-` |
| `OPENAI_BASE_URL` | *(Optional)* Custom endpoint for compatible services |

**Supported models**: gpt-image-1.5, dall-e-3

**Compatible services**: Set `OPENAI_BASE_URL` to use third-party services like Together AI, Fireworks AI, or any endpoint that follows the OpenAI image generation API format.
...
<Note>
  ComfyUI generates images one at a time (serial processing). Cloud providers can run up to 4 images in parallel.
</Note>
