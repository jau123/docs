...
<Tip>
  Prefer zero installation? The [remote MCP endpoint](/en/mcp/remote) needs no Node.js, no `npx`, and no local config file — just point any MCP host at `https://www.meigen.ai/api/mcp` over HTTP.
</Tip>
...
<Note>
  Also listed on the community [wshobson/agents](https://github.com/wshobson/agents) marketplace. That listing doesn't bundle MCP server config — after installing, add the JSON block from [Other MCP-Compatible Hosts](#other-mcp-compatible-hosts) to `.mcp.json` yourself.
</Note>
...
<Warning>
  ...
  If a session does disconnect mid-generation, don't resubmit — call `check_generation` with the `generationId` from your original request to resume tracking it. Resubmitting starts a new generation and deducts credits again.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Generation timeout" icon="circle-exclamation">
    ...
    If your session disconnects or polling gets interrupted, don't resubmit the same request — call `check_generation` with the `generationId` from your original request to resume tracking it. Resubmitting starts a new generation and deducts credits again.
