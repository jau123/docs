...
<ResponseField name="capabilities" type="object">
  Stable, forward-compatible capability contract. `capabilities.video` describes a video model's shape — output duration options, reference-video continuation range (seconds and upload size), and whether a first frame is required; `null` for image models. **New integrations should read `capabilities.video` instead of parsing `extra_config`** — the latter is kept only for backward compatibility with older clients.
</ResponseField>
...
<ResponseField name="extra_config" type="object">
  ...
  | `pricingPerSec` | Seedance 2.0 / Grok Video 1.5 / Agnes Video 2.0 | Per-second rate, keyed by resolution — or by tier then resolution on models that expose tiers |
...
<Note>
  Example shows two representative models with a partial `extra_config` / `capabilities.video` for brevity. The live endpoint returns all active models with their full configuration. Responses are cached for 1 hour (plus stale-while-revalidate), so newly added or retired models can take up to an hour to appear.
</Note>
