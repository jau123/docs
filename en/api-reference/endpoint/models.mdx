...
<ResponseField name="extra_config" type="object">
  ...
  | --- | --- | --- |
  | `resolutions` | Image / video | Allowed values for `resolution` (`"2K"` / `"3K"` / `"4K"` for image, `"480p"` / `"720p"` for video) |
  | `durations` | Seedance 2.0 | Allowed values for `duration` (seconds) |
  | `pricingPerSec` | Seedance 2.0 | Per-resolution per-second rate, e.g. `{ "480p": 13, "720p": 26 }` |
