...
<ParamField body="modelId" type="string">
  The model to use for generation. If omitted, the platform default is used (may change over time). Get available models from the [List Models](/en/api-reference/endpoint/models) endpoint.

  Image models: `gpt-image-2`, `gemini-3-pro-image-preview`, `nanobanana-2`, `seedream-5.0-lite`, `seedream-4.5`, `midjourney-v8.1`, `midjourney-niji7`, `z-image-turbo`

  Video models: `seedance-2-0` (Fast tier) / `seedance-2-0-pro` (Pro tier — higher quality), `happyhorse-1.0`, `veo-3.1`
...
<ParamField body="resolution" type="string">
  ...
  Available values depend on the model: image models support `1K` / `2K` / `3K` / `4K`; video models support `480p` / `720p` / `1080p`. See [Models](/en/features/models) for each model's supported resolutions and defaults.
</ParamField>
...
<ParamField body="referenceImages" type="string[]">
  ...
  Maximum count is per-model — see `max_reference_images` from [List Models](/en/api-reference/endpoint/models). Z Image Turbo accepts 0; Midjourney V8.1 / Niji 7 / Happyhorse 1.0 accept 1; most others accept up to 5.
</ParamField>

<ParamField body="niji7Options" type="object">
  Advanced parameters for Midjourney V8.1 / Niji 7. Ignored for other models. (Field name preserved for backward compatibility.)

  | Field | Type | Default | Range | Notes |
  | --- | --- | --- | --- | --- |
  | stylize | number | 100 (Niji 7) / 0 (V8.1) | 0–1000 | |
  | chaos | number | 0 | 0–100 | |
  | weird | number | 0 | 0–3000 | Niji 7 only (V8.1 ignores) |
  | raw | boolean | false | — | |
  | iw | number | 1 | 0–2 | |
  | sref | string | — | URL or text | |
  | sw | number | 100 | 0–1000 | |
  | sv | number | 4 (Niji 7) / 6 (V8.1) | 1–4 (Niji 7) / 6 (V8.1) | |
  | quality | string | `4` | `1` or `4` | V8.1 only — defaults to High (4× compute, no extra cost) |
</ParamField>

<ParamField body="referenceType" type="string" default="content">
  For Midjourney V8.1 / Niji 7 only. How to interpret the reference image.
...
<ParamField body="duration" type="number">
  Video duration in seconds. Seedance 2.0: 4–15 (default 5). Happyhorse 1.0: 3–15 (default 5). Veo 3.1 is fixed at 8 seconds and ignores this parameter.
</ParamField>
...
<ParamField body="referenceVideo" type="string">
  ...
  When this field is present, billing uses the "with reference video" tier: 480p 8 credits/sec, 720p 14 credits/sec, 1080p 14 credits/sec, with the minimum-billable-seconds floor applied (see [Models](/en/features/models)).
</ParamField>
...
<Note>
  ...
  - Midjourney V8.1 / Niji 7 return 4 candidate images per generation. `imageUrls` contains all candidates; `imageUrl` always points to the first one. Other image models return a single image.
</Note>
