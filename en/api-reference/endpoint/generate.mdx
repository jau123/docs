...
<ParamField body="referenceImages" type="string[]">
  ...
  | --- | --- |
  | 10 | Seedream 5.0 Pro |
  | 5 | GPT Image 2.0, Nanobanana 2, Nanobanana Pro, Seedream 4.5 |
  ...
  In `content` mode, Seedance 2.5 accepts up to 30 images through the API and Seedance 2.0 accepts up to 9. The MeiGen Web and App interfaces intentionally expose a smaller two-image product workflow.
</ParamField>
...
<ParamField body="referenceVideo" type="string">
  ...
  For both models, `billable seconds = max(reference duration + output duration, minimum-billable floor)`. See [Models](/en/features/models) for the complete rate tables and examples. The system detects the reference video's actual duration automatically.
</ParamField>
