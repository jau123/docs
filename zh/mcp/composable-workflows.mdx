...
本地要求 **`meigen@2.0.1` 或更高版本**（`npx -y meigen@2.0.1`），远程使用已更新的 **`https://www.meigen.ai/api/mcp`**。npm 1.4.0 不支持本页合同。发布顺序为后端部署、npm 发布、公开安装指南；2.0.1 尚未发布时应使用本地构建包验证。重新连接并检查实际 schema 是否包含 `requestId`、`wait` 和按请求查询。后端回滚时应保留恢复端点及幂等 POST 合同；无需撤回用户已安装的 npm 包。
