...
本地要求 **`meigen` 2.0.1 或更高版本**（`npx -y meigen@2.0.2`），远程使用已更新的 **`https://www.meigen.ai/api/mcp`**。npm 1.4.0 不支持本页合同。发布顺序为后端部署、npm 发布、公开安装指南；2.0.1 尚未发布时应使用本地构建包验证。重新连接并检查实际 schema 是否包含 `requestId`、`wait` 和按请求查询。后端回滚时应保留恢复端点及幂等 POST 合同；无需撤回用户已安装的 npm 包。
...
2. 每次新提交前，按当前选定模型、档位、时长和参考素材价格预留预计费用，同时计入全部在途预留。视频的预估必须按**所有参考视频时长之和** + 输出时长来算，因为计费秒数就是由这个总和得出的；参考音频免费，不占预算。返回后用实际 `creditsUsed` 和已确认退款核算；费用未明确时继续保留预算。
...
const video = await client.callTool({
  ...
  arguments: {
    ...
    // 参考素材都是数组。本地 npm 这里也接受本地文件路径；
    // 远程 MCP 只收 images.meigen.ai 的地址 —— 其他域名一律在扣点前 400。
    // 计划里没有对应素材时整个省略该字段 —— 这些字段属于 requestId 输入指纹的一部分。
    referenceVideos: script.referenceVideoUrls, // 按各条时长「总和」计费
    referenceAudios: script.referenceAudioUrls, // 免费，从不计费
