...
<ParamField body="modelId" type="string" default="gpt-image-2">
  ...
  图片模型：`gpt-image-2`（默认）, `gemini-3-pro-image-preview`, `nanobanana-2`, `seedream-5.0-lite`, `seedream-4.5`, `midjourney-v7`, `midjourney-niji7`, `z-image-turbo`

  视频模型：`seedance-2-0`, `veo-3.1`

  > `rhart-1.5`（GPT image 1.5）已于 2026-04-22 下线，请改用 `gpt-image-2`。
</ParamField>
...
<ParamField body="resolution" type="string" default="2K">
  ...
  可选值取决于模型：图片模型支持 `1K` / `2K` / `3K` / `4K`；视频模型支持 `480p` / `720p`。各模型实际支持的分辨率详见[模型对比](/zh/features/models)。
</ParamField>

<ParamField body="quality" type="string" default="medium">
  仅用于 GPT Image 2.0。可选 `low` 或 `medium`。其他模型会忽略该参数。
</ParamField>

<ParamField body="referenceImages" type="string[]">
  用作参考的图片 URL 数组。必须是可公开访问的 HTTPS URL。

  最大数量按模型不同——参考[模型列表](/zh/api-reference/endpoint/models)的 `max_reference_images` 字段。Z Image Turbo 不支持参考图；Midjourney V7 / Niji 7 仅支持 1 张；其他模型最多 5 张。
</ParamField>
