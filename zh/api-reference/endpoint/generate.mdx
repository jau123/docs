...
<ParamField body="prompt" type="string" required>
  ...
  支持范围:所有接受参考图的图片模型(除 Flux 2 Klein、Z Image Turbo 外)+ 视频侧的 Seedance 2.0。模型不支持、角色不存在或无权访问、引用数超限均返回 `400`(`characters_not_supported`、`character_not_found`、`too_many_characters`);部分角色需要更高的 `tier`(`character_tier_required`)。
</ParamField>
...
<ParamField body="resolution" type="string">
  ...
  可选值取决于模型：图片模型用 `1K` / `2K` / `3K` / `4K`；视频模型用 `480p` / `720p` / `1080p` / `4k`（视频侧的 k 是小写）。并非每个视频模型都支持全部取值 —— `4k` 仅 Seedance 2.0 的 `pro` 档和 Veo 3.1 支持，Seedance 上的 `1080p` 同样需要 `pro` 档。各模型实际支持的分辨率和默认值详见[模型对比](/zh/features/models)。带 `referenceVideo` 续写时（Seedance 2.0），上限为 `1080p` —— `4k` 仅对不带参考视频的生成开放。
</ParamField>
...
<ParamField body="referenceImages" type="string[]">
  ...
  | 2 | Seedance 2.0、Veo 3.1 —— 默认 `frame` 模式(首/尾帧),见下方 `referenceMode` |
  | 1 | Midjourney V8.1、Agnes Image 2.1 Flash、Agnes Video 2.0、Grok Video 1.5(**必传** —— 仅支持图生视频) |
  | 0 | Flux 2 Klein、Z Image Turbo |

  Seedance 2.0 上,将 `referenceMode` 切到 `content` 可将上限提高到 9。
</ParamField>

<ParamField body="referenceMode" type="string" default="frame">
  仅用于视频模型。决定 `referenceImages` 的解读方式。

  - `frame`(默认)—— 首/尾帧参考,最多 2 张
  - `content` —— 内容参考,不绑定具体帧位置。Seedance 2.0 最多 9 张;其他视频模型仍沿用 `frame` 模式的 2 张上限

  超限返回 `400` —— `frame` 模式下为 `frame_reference_limit`,`content` 模式下为 `too_many_references`。
</ParamField>
...
<ParamField body="referenceVideo" type="string">
  ...
  上传的参考视频须 **2-15 秒**、文件 **不超过 50MB**，超出范围返回 `400` / `invalid_reference_video`。续写模式下输出分辨率最高 `1080p` —— Mini、Fast 支持 `480p`/`720p`，Pro 支持 `720p`/`1080p`；`4k` 仅对不带参考视频的生成开放。

  传该字段时计费走"有参考视频"档（Mini 480p 7、720p 13；Fast 480p 8、720p 14；Pro 720p 16、1080p 28 积分/秒），按"参考视频 + 输出"合计秒数计算，并应用最低计费秒数（详见[模型](/zh/features/models)）。参考视频的实际时长由系统自动识别用于该计算——见下方 `referenceVideoDuration`。
</ParamField>

<ParamField body="referenceVideoDuration" type="number">
  可选，仅用于 Seedance 2.0 + 参考视频续写。系统会自动识别上传的参考视频的真实时长并用于计费与裁剪，无需设置该字段。若传了，仅作为与识别值的诊断性比对，其余情况忽略。
</ParamField>
...
计费：这里没有传 `tier`，因此走默认 **Mini** 档 —— 480p 带参考视频单价 7 积分/秒。假设上传的参考视频实际时长为 3 秒（系统自动识别），计费秒数 = max(3 + 5, 9) = 9 秒，共 **63 积分**。同样的请求走 `fast` 档则是 8 × 9 = 72 积分。输出视频长度 = `duration` = 5 秒。
