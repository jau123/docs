...
<ParamField body="prompt" type="string" required>
  ...
  支持范围：所有接受参考图的图片模型（除 Flux 2 Klein、Z Image Turbo 外）+ 视频侧**仅 Seedance 2.0**。Seedance 2.5 不支持角色资产。模型不支持、角色不存在或无权访问、引用数超限均返回 `400`（`characters_not_supported`、`character_not_found`、`too_many_characters`）；部分角色需要更高的 `tier`（`character_tier_required`）。
</ParamField>
...
<ParamField body="referenceImages" type="string[]">
  ...
  `content` 模式下，Seedance 2.5 的 API 最多接受 30 张图片，Seedance 2.0 最多 9 张。MeiGen Web 与 App 界面出于产品体验仍只开放两张内容参考图。
</ParamField>
...
<ParamField body="referenceVideo" type="string">
  ...
  两个模型都按 `计费秒数 = max(参考视频时长 + 输出时长, 最低计费秒数)`。完整费率和示例见[模型](/zh/features/models)。系统会自动识别参考视频真实时长。
</ParamField>
